    ...
    <body>
                    ...
                    <div class="d-flex">
                        <div class="sb2-2-2">
                            <ul>
                                ...
                                <li class="active-bre"><a> CLO List</a></li>
                            </ul>
                        </div>
                        <div class="float-right">
                            <a href="cloDetail?type=add"><button class="btn btn-warning">Add CLO</button></a>
                        </div>
                    </div>
                                                ...
                                                <table id="" class="table table-hover" style="width:100%">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th onclick="sortByType('${sort == "" ? 'syllabusName_down' : sort eq 'syllabusName_up' ? 'syllabusName_down': 'syllabusName_up'}')">Syllabus Name <span class="fa ${sort == "" ? 'fa-angle-up' : sort eq 'syllabusName_up' ? 'fa-angle-up' : 'fa-angle-down'}"></span></th>
                                                            ...
                                                            <th >Active <span class="fa ${sort == "" ? 'fa-angle-up' : sort eq '' ? 'fa-angle-up' : 'fa-angle-down'}"></span></th>
                                                            <th></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <c:forEach items="${list}" var="item">
                                                            <tr>
                                                                <td>${item.loid}</td>
                                                                <td>${custom.getSyllabusNameENByID(item.loid)}</td>

                                                                <td>${item.cloDetail}</td>
                                                                <td><span class="fa ${item.isActive ? 'fa-check text-success' : 'fa-times text-danger'}"></span></td>
                                                                <td>
                                                                    <a href="cloDetail?loid=${item.loid}" >View</a>
                                                                </td>
                                                            </tr>
                                                        </c:forEach>
        ...
        <script>
                                                                ...
                                                                function changePage(page, sort) {
                                                                    let key = document.getElementById("search").value;
                                                                    let url = './cloList?keySearch=' + key + "&sort=" + sort + "&page=" + page;
                                                                ...
                                                                function sortByType(type) {
                                                                    let key = document.getElementById("search").value;
                                                                    let url = './cloList?keySearch=' + key + "&sort=" + type;
